...
class nanopool():
    ...
    def plot_chart(self, ticker):
        ...
        for i in range(len(data)):
            t=np.append(t,datetime.datetime.fromtimestamp(data[i]['date']).strftime('%m-%d %H:%M'))
            # t=np.append(t,data[i]['date'])
            shares=np.append(shares,data[i]['shares'])
            hr=np.append(hr,data[i]['hashrate']/1000.)
            
        for i in range(len(data2)):
            t2 = np.append(t2, datetime.datetime.fromtimestamp(data2[i]['date']).strftime('%m-%d %H:%M'))
        ...
        if ticker == 'eth':
            ...
            ax1.legend(['Reported hashrate','Actual hashrate','2-hr moving average'])
        else:
            mahr=self.TI.MAlist(hr2,12)
            ax1.plot(t2[-72:],hr2[-72:])
            ax1.plot(t2[-72:],mahr[-72:])
            ax1.legend(['Actual hashrate','2-hr moving average'])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        nargs='?',
        default='eth',
        help='Cryptocurrency name.'
    )
    args = parser.parse_args()
    print(nanopool().plot_chart(args.ticker))